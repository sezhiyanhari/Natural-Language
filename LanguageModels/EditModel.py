...
class EditModel(object):
  ...
  def insertEdits(self, word):
    ...
    for c in self.ALPHABET:
      for i in xrange(1, len(word)):
        # The corrupted signal is the current letter
        corruptLetters = word[i-1]        
        # unsure of where to get next character
        correctLetters = corruptLetters + c
        correction = "%s%s%s" % (word[0:i-1], correctLetters, word[i:len(word)-1])
  ...
  def transposeEdits(self, word):
    ...
    for i in xrange(1, len(word) - 1):
      # The corrupted signal is this letter and the previous
      corruptLetters = word[i:i+2]
      # The correct signal is this letter and previous, reversed
      correctLetters = corruptLetters[1] + corruptLetters[0]
    ...
    for i in xrange(0, len(word)):
      for c in self.ALPHABET:
        ...
        if (corruptLetters == correctLetters):
          continue
        correction = "%s%s%s" % (word[0:i], correctLetters, word[i+1:])
        ret.append(Edit(correction, corruptLetters, correctLetters))
